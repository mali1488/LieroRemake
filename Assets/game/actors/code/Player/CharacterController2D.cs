...
public class CharacterController2D : MonoBehaviour {
  ...
  private const float SafetyMargin = .0001f;
  ...
  private void CalculateRayOrigins() {
    ...
    var center = new Vector2(_boxCollider.offset.x/2 * _localScale.x, _boxCollider.offset.y/2 * _localScale.y);
  ...
  private void MoveHorizontally(ref Vector2 deltaMovement) {
    ...
    for (var i = 0; i < TotalVerticalRays; i++) {
      ...
      if (rayDistance < SkinWidth + SafetyMargin) {
        break;
      }
    }
  }
  ...
  private void MoveVertically(ref Vector2 deltaMovement) {
    ...
    for (var i = 0; i < TotalVerticalRays; i++) {
      ...
      if (!isGoingUp && deltaMovement.y > SafetyMargin) {
  ...
  private void HandleVerticalSlope(ref Vector2 deltaMovement) {
    ...
    Debug.Log("HandleVerticalSlope() angle: " + angle + " dy: " + deltaMovement.y + " dx: " + deltaMovement.x);

    if (Mathf.Abs(angle) < SafetyMargin) {
      return;
    }
  ...
  private bool HandleHorizontalSlope(ref Vector2 deltaMovement, float angle, bool isGoingRight) {
    Debug.Log("HandleHorizontalSlope: " + angle + " dy: " + deltaMovement.y + " dx: " + deltaMovement.x);
